...
def _pure_geometry_checks(vertices: Sequence[Vec3], faces: Sequence[Face]) -> Dict[str, Any]:
    ...
    stats.update({"source_face_count": len(faces), "triangles": len(triangles), "invalid_face_count": int(invalid_faces), "zero_area_faces": int(degenerate_faces), "duplicate_triangle_count": int(_duplicate_triangle_count(triangles)), "component_count_python": int(_component_count(triangles))})
    return stats
